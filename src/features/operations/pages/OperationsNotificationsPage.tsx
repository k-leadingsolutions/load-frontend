...
export const OperationsNotificationsPage = () => {
  ...
  const notificationsQuery = useQuery({
    ...
    queryFn: () => mockNotificationService.listNotifications('OPERATIONS'),
  })
  const markAllRead = useMutation({
    mutationFn: () => mockNotificationService.markAllRead('OPERATIONS'),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['operations-notifications'] }),
  })
  ...
  return (
    <SectionCard
      ...
      action={(
        <button type="button" onClick={() => markAllRead.mutate()} className="rounded-pill border border-card-border px-4 py-2 text-sm font-semibold text-load-700">
          Mark all read
        </button>
      )}
    >
      {notifications.length === 0 ? <EmptyState title="No notifications" description="System updates will appear here." /> : (
